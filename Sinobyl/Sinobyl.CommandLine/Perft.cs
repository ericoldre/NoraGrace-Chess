...
namespace Sinobyl.CommandLine
{
    public static class Perft
    {
        ...
        private static TimeSpan PerftTest(string fen, int nodeCount, bool doEval, bool doMoveSort)
        {
            ...
            ChessMove[][] moveStack = ChessMoveInfo.InitalizeMoveStack(30);
            ...
            {
                PerftSearch(0, board, depth, nodeCount, ref nodesDone, doEval, doMoveSort, moveStack);
        ...
        public static void PerftSearch(int ply, ChessBoard board, int depth_remaining, int nodeCount, ref int nodesDone, bool doEval, bool doMoveSort, ChessMove[][] moveStack)
        {
            nodesDone++;
            if (doEval)
            {
            ...
            ChessMove[] moves = moveStack[ply];
            var moveCount = ChessMoveInfo.GenMovesToArray(board, moves, 0, false);
            ...
            for (int i = 0; i < moveCount; i++)
            {
                ...
                {
                    PerftSearch(ply + 1, board, depth_remaining - 1, nodeCount, ref nodesDone, doEval, doMoveSort, moveStack);
                }
